...
void token::transfers( const name&    from,
                       const name&    to,
                       const std::vector<asset>&   quantitys,
                       const string&  memo )
{
    ...
    for (int i=0; i<quantitys.size(); i++) {
        ...
        auto sym = quantitys[i].symbol.code();
        stats statstable( get_self(), sym.raw() );
        const auto& st = statstable.get( sym.raw(), "no balance with specified symbol" );
        ...
        check( quantitys[i].is_valid(), "invalid quantity" );
        check( quantitys[i].amount > 0, "must transfer positive quantity" );
        check( quantitys[i].symbol == st.supply.symbol, "symbol precision mismatch" );
        check( memo.size() <= 256, "memo has more than 256 bytes" );

        auto payer = has_auth( to ) ? to : from;

        sub_balance( from, quantitys[i] );
        add_balance( to, quantitys[i], payer );
...
void token::airgrab(const name &owner) 
{
   ...
   //   auto sym = symbol("SBM", 4); // NEWT is the token symbol with precision 4
   ...
   //   auto sym2 = symbol("SFOOD", 4); // NEWT is the token symbol with precision 4
   //   asset airgrabbed_asset2(100000, sym2);    // allow 10 tokens to be airgrabbed

   //   sub_balance(_self, airgrabbed_asset2);
   //   add_balance(owner, airgrabbed_asset2, owner);
